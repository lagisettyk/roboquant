...
class EMATrend(strategy.BacktestingStrategy):
	...
	def initLogging(self):
		...
		logFileName = "EMA_Trend_" + self.__instrument + ".log"
	...
	def onBars(self, bars):
		...
		self.__highDS = feedLookbackEndAdj.getHighDataSeries(self.__instrumentAdj)
		...
		if self.exitLongSignal(bar):
			...
			if self.enterLongSignal(bar):
				...
				entryPrice = 0.0
				entryPriceDelta = 0.0
				if "OR" in self.__inpEntry["EMA_Trend_Call"] and "Breach" in self.__inpEntry["EMA_Trend_Call"]["OR"]:
					if consts.EMA_ENTRY_PRICE_DELTA_PERCENT_OR_ABS.lower() == 'percent':
						entryPriceDelta = bar.getClose() * consts.EMA_ENTRY_PRICE_DELTA_PERCENT / float(100)
					else:
						entryPriceDelta = consts.EMA_ENTRY_PRICE_DELTA_ABS
				...
				entryPrice = 0.0
				entryPriceDelta = 0.0
				if "OR" in self.__inpEntry["EMA_Trend_Put"] and "Breach" in self.__inpEntry["EMA_Trend_Put"]["OR"]:
					if consts.EMA_ENTRY_PRICE_DELTA_PERCENT_OR_ABS.lower() == 'percent':
						entryPriceDelta = bar.getClose() * consts.EMA_ENTRY_PRICE_DELTA_PERCENT / float(100)
					else:
						entryPriceDelta = consts.EMA_ENTRY_PRICE_DELTA_ABS
	...
	def enterLongSignal(self, bar):
		...
		if "Candle_Len_Check" in self.__inpStrategy["EMA_Trend_Call"] and "Against_Average_Length" in self.__inpStrategy["EMA_Trend_Call"]["Candle_Len_Check"]:
		...
		if "Wick_Rel_To_Candle" in self.__inpStrategy["EMA_Trend_Call"] and "Cutoff_Check" in self.__inpStrategy["EMA_Trend_Call"]["Wick_Rel_To_Candle"]:
			wickLen = bar.getHigh() - bar.getClose()
			candleLen = bar.getClose() - bar.getOpen()
			if candleLen == 0:
				candleLen = consts.DUMMY_CANDLE_LEN
		...
		# Detect the EMA bounce with the following:
		# 1. Low price within bounce delta of EMA
		# 2. While close price is on the same side of the EMA, look for the first
		#    BB upper band breach.
		# 3. Then, look for the first EMA breach by the close price going 
		#    further back.
		if self.__inpStrategy["EMA_Trend_Call"]["EMA"]["AND"][0] == "Bounce":
			closePriceArray = xiquantFuncs.dsToNumpyArray(self.__closeDS, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revClosePriceArray = xiquantFuncs.revNumpyArray(closePriceArray)
			emaArray = xiquantFuncs.dsToNumpyArray(self.__emaDS, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revEMAArray = xiquantFuncs.revNumpyArray(emaArray)
			upperBBArray = xiquantFuncs.dsToNumpyArray(self.__upperBBDataSeries, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revUpperBBArray = xiquantFuncs.revNumpyArray(upperBBArray)
			firstEMACrossingByPrice = xiquantFuncs.firstNumpyArrayCrossing(revClosePriceArray, revEMAArray)
			firstBBUpperCrossingByPrice = xiquantFuncs.firstNumpyArrayCrossing(revClosePriceArray, revUpperBBArray)
			if self.__lowDS[-1] > self.__emaDS[-1] * ((float(100) + consts.EMA_TREND_BOUNCE_DELTA_PERCENT) / float(100)) or self.__lowDS[-1] < self.__emaDS[-1] * ((float(100) - consts.EMA_TREND_BOUNCE_DELTA_PERCENT) / float(100)):
				self.__logger.debug("Bounce check failed: low price NOT within the bounce delta.")
				return False
			if firstBBUpperCrossingByPrice == 0:
				self.__logger.debug("Bounce check failed: No BB Upper Band breach in the lookback.")
				return False
			if firstEMACrossingByPrice <= firstBBUpperCrossingByPrice:
				self.__logger.debug("Bounce check failed: price not on the same side of EMA before BB Upper Band breach.")
				return False
			if firstEMACrossingByPrice == 0:
				furtherBackClosePriceArray = xiquantFuncs.dsToNumpyArray(self.__closeDS, consts.EMA_TREND_BB_BREACH_LOOKBACK + consts.EMA_TREND_FURTHER_BREACH_LOOKBACK)
				revFurtherBackClosePriceArray = xiquantFuncs.revNumpyArray(furtherBackClosePriceArray)
				furtherBackEMAArray = xiquantFuncs.dsToNumpyArray(self.__emaDS, consts.EMA_TREND_BB_BREACH_LOOKBACK + consts.EMA_TREND_FURTHER_BREACH_LOOKBACK)
				revFurtherBackEMAArray = xiquantFuncs.revNumpyArray(furtherBackEMAArray)
				firstFurtherBackEMACrossingByPrice = xiquantFuncs.firstNumpyArrayCrossing(revFurtherBackClosePriceArray, revFurtherBackEMAArray)
				if firstFurtherBackEMACrossingByPrice == 0:
					self.__logger.debug("Bounce check failed: No close price crossover of EMA in further back lookback.")
					return False

				self.__logger.debug("EMA bounce check passed.")

		# Check the price jump
		if "Price_Jump" in self.__inpStrategy["EMA_Trend_Call"] and "Price_Jump_Check" in self.__inpStrategy["EMA_Trend_Call"]["Price_Jump"]:
		...
		# Check volume 
		if "Volume" in self.__inpStrategy["EMA_Trend_Call"] and "Volume_Check" in self.__inpStrategy["EMA_Trend_Call"]["Volume"]:
		...
		# Check cash flow 
		if "Cash_Flow" in self.__inpStrategy["EMA_Trend_Call"] and "Cash_Flow_Check" in self.__inpStrategy["EMA_Trend_Call"]["Cash_Flow"]:
		...
		# Check resistance
		if "Resistance_Or_Support" in self.__inpStrategy["EMA_Trend_Call"] and "Resistance_Check" in self.__inpStrategy["EMA_Trend_Call"]["Resistance_Or_Support"]:
			if (len(self.__priceDS) < consts.TRADE_DAYS_IN_RESISTANCE_LOOKBACK_WINDOW):
		...
		closePrice = bar.getClose()
		if "Averages" in self.__inpStrategy["EMA_Trend_Call"] and "AND" in self.__inpStrategy["EMA_Trend_Call"]["Averages"]:
			if "AND" in self.__inpStrategy["EMA_Trend_Call"]["Averages"] and "Price_Check" in self.__inpStrategy["EMA_Trend_Call"]["Averages"]["AND"]:
		...
		if "Averages" in self.__inpStrategy["EMA_Trend_Call"] and "AND" in self.__inpStrategy["EMA_Trend_Call"]["Averages"]:
			if "AND" in self.__inpStrategy["EMA_Trend_Put"]["Averages"] and "Wick_Top_Check" in self.__inpStrategy["EMA_Trend_Put"]["Averages"]["AND"]:
				wick = bar.getHigh()
		...
		if "RSI" in self.__inpStrategy["EMA_Trend_Call"] and "AND" in self.__inpStrategy["EMA_Trend_Call"]["RSI"]:
			...
			if "AND" in self.__inpStrategy["EMA_Trend_Call"]["RSI"] and "Pointing_Up" in self.__inpStrategy["EMA_Trend_Call"]["RSI"]["AND"]:
				if rsiArrayInLookback[-1] != rsiArrayInLookback.max():
					self.__logger.debug("RSI lookback check failed.")
					return False
			#if "AND" in self.__inpStrategy["EMA_Trend_Call"]["RSI"] and "Greater_Than_Oversold" in self.__inpStrategy["EMA_Trend_Call"]["RSI"]["AND"]:
		...
		if "MACD" in self.__inpStrategy["EMA_Trend_Call"] and "No_Divergence" in self.__inpStrategy["EMA_Trend_Call"]["MACD"]:
			if len(self.__priceDS) < consts.MACD_PRICE_DVX_LOOKBACK:
		...
		# Check DMI+ and DMI-
		if "ADX_And_DMI" in self.__inpStrategy["EMA_Trend_Call"] and "AND" in self.__inpStrategy["EMA_Trend_Call"]["ADX_And_DMI"]:
			...
			if "AND" in self.__inpStrategy["EMA_Trend_Call"]["ADX_And_DMI"] and "DMI_Plus_Higher" in self.__inpStrategy["EMA_Trend_Call"]["ADX_And_DMI"]["AND"]:
				if (self.__dmiPlus[-1] <= self.__dmiPlus[-2]):
					self.__logger.debug("DMI Plus not pointing up.")
					return False
			if "AND" in self.__inpStrategy["EMA_Trend_Call"]["ADX_And_DMI"] and "DMI_Minus_Lower" in self.__inpStrategy["EMA_Trend_Call"]["ADX_And_DMI"]["AND"]:
	...
	def enterShortSignal(self, bar):
		...
		# Check if the candle length is greater than the cutoff when compared
		# to the average candle length of past days.
		if "Candle_Len_Check" in self.__inpStrategy["EMA_Trend_Put"] and "Against_Average_Length" in self.__inpStrategy["EMA_Trend_Put"]["Candle_Len_Check"]:
			openPriceArrayInLookback = xiquantFuncs.dsToNumpyArray(self.__openDS, consts.CANDLE_LEN_CHECK_LOOKBACK_WINDOW)
			closePriceArrayInLookback = xiquantFuncs.dsToNumpyArray(self.__closeDS, consts.CANDLE_LEN_CHECK_LOOKBACK_WINDOW)
		...
		if "Wick_Rel_To_Candle" in self.__inpStrategy["EMA_Trend_Put"] and "Cutoff_Check" in self.__inpStrategy["EMA_Trend_Put"]["Wick_Rel_To_Candle"]:
			wickLen = bar.getClose() - bar.getLow()
			candleLen = bar.getOpen() - bar.getClose()
			# Relative wick length as a percentage of the candle length
			if candleLen == 0:
		...
		# 3. Then, look for the first EMA breach by the close price going 
		#    further back.
		if self.__inpStrategy["EMA_Trend_Put"]["EMA"]["AND"][0] == "Bounce":
			closePriceArray = xiquantFuncs.dsToNumpyArray(self.__closeDS, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revClosePriceArray = xiquantFuncs.revNumpyArray(closePriceArray)
			emaArray = xiquantFuncs.dsToNumpyArray(self.__emaDS, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revEMAArray = xiquantFuncs.revNumpyArray(emaArray)
			lowerBBArray = xiquantFuncs.dsToNumpyArray(self.__lowerBBDataSeries, consts.EMA_TREND_BB_BREACH_LOOKBACK)
			revLowerBBArray = xiquantFuncs.revNumpyArray(lowerBBArray)
			firstEMACrossingByPrice = xiquantFuncs.firstNumpyArrayCrossing(revClosePriceArray, revEMAArray)
			firstBBLowerCrossingByPrice = xiquantFuncs.firstNumpyArrayCrossing(revClosePriceArray, revLowerBBArray)
			if self.__highDS[-1] < self.__emaDS[-1] * ((float(100) - consts.EMA_TREND_BOUNCE_DELTA_PERCENT) / float(100)) or self.__highDS[-1] > self.__emaDS[-1] * ((float(100) + consts.EMA_TREND_BOUNCE_DELTA_PERCENT) / float(100)):
				self.__logger.debug("Bounce check failed: high price NOT within the bounce delta.")
				return False
			if firstBBLowerCrossingByPrice == 0:
				self.__logger.debug("Bounce check failed: No BB Lower Band breach in the lookback.")
				return False
			if firstEMACrossingByPrice <= firstBBLowerCrossingByPrice:
				self.__logger.debug("Bounce check failed: price not on the same side of EMA before BB Lower Band breach.")
				return False
			if firstEMACrossingByPrice == 0:
				furtherBackClosePriceArray = xiquantFuncs.dsToNumpyArray(self.__closeDS, consts.EMA_TREND_BB_BREACH_LOOKBACK + consts.EMA_TREND_FURTHER_BREACH_LOOKBACK)
				revFurtherBackClosePriceArray = xiquantFuncs.revNumpyArray(furtherBackClosePriceArray)
		...
		# Check the price jump
		# +1 because we need one additional entry to compute the candle jump
		if "Price_Jump" in self.__inpStrategy["EMA_Trend_Put"] and "Price_Jump_Check" in self.__inpStrategy["EMA_Trend_Put"]["Price_Jump"]:
			if (len(self.__closeDS) < consts.PRICE_JUMP_LOOKBACK_WINDOW + 1):
				self.__logger.debug("Not enough entires for Price Jump check lookback.")
				self.__logger.debug("Lookback: %d" % consts.PRICE_JUMP_LOOKBACK_WINDOW)
				self.__logger.debug("Entries: %d" % len(self.__closeDS))
		...
		# Check volume 
		if "Volume" in self.__inpStrategy["EMA_Trend_Put"] and "Volume_Check" in self.__inpStrategy["EMA_Trend_Put"]["Volume"]:
			if (len(self.__volumeDS) < consts.VOLUME_LOOKBACK_WINDOW) or (len(self.__volumeDS) < consts.VOLUME_AVG_WINDOW):
		...
		# Check cash flow 
		if "Cash_Flow" in self.__inpStrategy["EMA_Trend_Put"] and "Cash_Flow_Check" in self.__inpStrategy["EMA_Trend_Put"]["Cash_Flow"]:
			if len(self.__priceDS) < consts.CASH_FLOW_LOOKBACK_WINDOW: 
				self.__logger.debug("Not enough entries for cashflow lookback")
				self.__logger.debug("Cashflow lookback: %d" % consts.CASH_FLOW_LOOKBACK_WINDOW)
				self.__logger.debug("Number of entries: %d" % len(self.__priceDS))
		...
		# Check support
		if "Resistance_Or_Support" in self.__inpStrategy["EMA_Trend_Put"] and "Support_Check" in self.__inpStrategy["EMA_Trend_Put"]["Resistance_Or_Support"]:
		...
		# Check price against the averages
		closePrice = bar.getClose()
		if "Averages" in self.__inpStrategy["EMA_Trend_Put"] and "AND" in self.__inpStrategy["EMA_Trend_Put"]["Averages"]:
			if "AND" in self.__inpStrategy["EMA_Trend_Put"]["Averages"] and "Price_Check" in self.__inpStrategy["EMA_Trend_Put"]["Averages"]["AND"]:
				if abs(closePrice - self.__ema1) < consts.PRICE_AVG_CHECK_DELTA:
					self.__logger.debug("Price comparison against EMA 10 failed.")
					self.__logger.debug("Price: %.2f" % closePrice)
					self.__logger.debug("EMA 10: %.2f" % self.__ema1)
		...
		if "RSI" in self.__inpStrategy["EMA_Trend_Put"] and "AND" in self.__inpStrategy["EMA_Trend_Put"]["RSI"]:
			if len(self.__rsi) < consts.RSI_SETTING:
				return False
			if (len(self.__rsi) < consts.RSI_LOOKBACK_WINDOW):
				self.__logger.debug("Not enough entries for RSI lookback")
				...
				return False
			rsiArrayInLookback = xiquantFuncs.dsToNumpyArray(self.__rsi, consts.RSI_LOOKBACK_WINDOW)
			if "AND" in self.__inpStrategy["EMA_Trend_Put"]["RSI"] and "Pointing_Down" in self.__inpStrategy["EMA_Trend_Put"]["RSI"]["AND"]:
				if rsiArrayInLookback[-1] != rsiArrayInLookback.min():
					self.__logger.debug("RSI lookback check failed.")
					return False
			#if "AND" in self.__inpStrategy["EMA_Trend_Put"]["RSI"] and "Less_Than_Overbought" in self.__inpStrategy["EMA_Trend_Put"]["RSI"]["AND"]:
		...
		# Check MACD, should show no divergence with the price chart in the lookback window
		if "MACD" in self.__inpStrategy["EMA_Trend_Put"] and "No_Divergence" in self.__inpStrategy["EMA_Trend_Put"]["MACD"]:
			if len(self.__priceDS) < consts.MACD_PRICE_DVX_LOOKBACK:
				self.__logger.debug("Not enough entries for MACD lookback")
				self.__logger.debug("MACD lookback: %d" % consts.MACD_PRICE_DVX_LOOKBACK)
				self.__logger.debug("Number of MACD entries: %d" % len(self.__priceDS))
		...
		# Check DMI+ and DMI-
		if "ADX_And_DMI" in self.__inpStrategy["EMA_Trend_Put"] and "AND" in self.__inpStrategy["EMA_Trend_Put"]["ADX_And_DMI"]:
			if len(self.__dmiPlus) <= consts.DMI_PERIOD or len(self.__dmiMinus) <= consts.DMI_PERIOD:
				self.__logger.debug("Not enough entries for DMI check")
				self.__logger.debug("DMI setting: %d" % consts.DMI_PERIOD)
				self.__logger.debug("Number of DMI entries: %d" % len(self.__dmiPlus))
				return False
			# Add the code to give higher priority for investment to cases when both the conditions are satisfied.
			if "AND" in self.__inpStrategy["EMA_Trend_Put"]["ADX_And_DMI"] and "DMI_Plus_Lower" in self.__inpStrategy["EMA_Trend_Put"]["ADX_And_DMI"]["AND"]:
				if (self.__dmiPlus[-1] >= self.__dmiPlus[-2]):
					self.__logger.debug("DMI Plus not pointing down.")
					return False
			if "AND" in self.__inpStrategy["EMA_Trend_Put"]["ADX_And_DMI"] and "DMI_Minus_Higher" in self.__inpStrategy["EMA_Trend_Put"]["ADX_And_DMI"]["AND"]:
				if (self.__dmiMinus[-1] <= self.__dmiMinus[-2]):
...
def run_strategy(bBandsPeriod, instrument, startPortfolio, startPeriod, endPeriod, plot=False):
	...
	strat = EMATrend(feedAdjustedToEndDate, feed, instrument, bBandsPeriod, earningsCalList, startPortfolio)
	strat.run()
...
def main(plot):
	...
	for inst in instruments:
		ret = run_strategy(bBandsPeriod, inst, startPortfolio, startDate, endDate, plot)
