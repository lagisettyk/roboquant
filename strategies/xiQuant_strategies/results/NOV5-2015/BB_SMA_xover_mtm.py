...
class BBSMACrossover(strategy.BacktestingStrategy):
	...
	def onBars(self, bars):
		...
		self.__sma10 = indicator.SMA(self.__closeDS, len(self.__closeDS), 10)
		self.__sma5 = indicator.SMA(self.__closeDS, len(self.__closeDS), 5)
		self.__sma40 = indicator.SMA(self.__closeDS, len(self.__closeDS), 40)
		...
		self.__macd = indicator.MACD(self.__closeDS, len(self.__closeDS), 18, 8, 6)
		...
		self.__adx = indicator.ADX(feedLookbackEndAdj.getBarSeries(self.__instrumentAdj), len(feedLookbackEndAdj.getBarSeries(self.__instrumentAdj)), consts.ADX_PERIOD)
	...
	def enterLongSignal(self, bar):
		...
		if self.__inpStrategy["BB_SMA_Crossover_Mtm_Call"]["SMA"]["AND"][1] == "First_Breach":
			...
			if self.__smaDS[-3] < self.__smaDS[-1]: 
				return False

			#### Additional confirmation of SMA trend based on most recent bars
			if self.__sma5[-1] < self.__smaDS[-1] or self.__sma10[-1] < self.__smaDS[-1] or self.__sma40[-1] < self.__smaDS[-1]:
				pass
			else:
				return False
		...
		if "MACD" in self.__inpStrategy["BB_SMA_Crossover_Mtm_Call"] and "No_Divergence" in self.__inpStrategy["BB_SMA_Crossover_Mtm_Call"]["MACD"]:
			...
			#### Check if MACD is greater than signal
			if self.__macd[1][-1] > 0.0:
				return False
	...
	def exitLongSignal(self, bar):
		...
		if "Volume_Price_Trend" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"] and "Divergence" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"]["Volume_Price_Trend"]:
			...
			volumeTrend = indicator.HT_TRENDLINE(self.__volumeDS, len(self.__volumeDS))
			priceTrend = indicator.HT_TRENDLINE(self.__closeDS, len(self.__closeDS))
		...
		if "RSI_Price_Trend" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"] and "Divergence" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"]["RSI_Price_Trend"]:
			...
			rsiTrend = indicator.HT_TRENDLINE(self.__rsi, len(self.__rsi))
			priceTrend = indicator.HT_TRENDLINE(self.__closeDS, len(self.__closeDS))
		...
		if "MACD_Price_Trend" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"] and "Divergence" in self.__inpExit["BB_SMA_Crossover_Mtm_Call"]["MACD_Price_Trend"]:
			...
			macdTrend = indicator.HT_TRENDLINE(self.__macd[0], consts.SMA_EXIT_MACD_PRICE_TREND_LOOKBACK_WINDOW)
