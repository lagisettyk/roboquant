...
def computeIndicators(instrument, startdate, enddate ):
    from pyalgotrade.talibext import indicator
    ...
    feedLookbackEndAdj.adjustBars()
    closeDS = feedLookbackEndAdj.getCloseDataSeries(instrument + "_adjusted")
    volumeDS = feedLookbackEndAdj.getVolumeDataSeries(instrument + "_adjusted")
    dateTimes = feedLookbackEndAdj.getDateTimes(instrument + "_adjusted")
    ...
    cashFlowToday =   (closeDS[-1] - closeDS[-2]) * volumeDS[-1]
    cashFlowPreviousDay =   (closeDS[-2] - closeDS[-3]) * volumeDS[-2]

    print volumeDS[-1], closeDS[-1], closeDS[-2], closeDS[-3], closeDS[-4]
    
    return cashFlowToday - cashFlowPreviousDay
    '''
    barDS = feedLookbackEndAdj.getBarSeries(instrument + "_adjusted")

    adx = indicator.ADX(barDS, len(barDS), 12)
    dmiPlus = indicator.PLUS_DI(barDS, len(barDS), 28)
    adx = indicator.ADX(barDS, len(barDS), 12)

    dateTimes = feedLookbackEndAdj.getDateTimes(instrument + "_adjusted")

    adxDS = numpy_to_highchartds(dateTimes, adx, startdate, enddate)
    dmiPlusDS = numpy_to_highchartds(dateTimes, dmiPlus, startdate, enddate)
...
def run_strategy_redis(bBandsPeriod, instrument, startPortfolio, startdate, enddate, filterCriteria=20, indicators=True):
    ...
    if indicators:
        ...
        #updatedOrders = updateOrdersRankbyMoneyFlowPercentChange(strat.getOrders(), instrument)
