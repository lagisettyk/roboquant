...
class MyStrategy(strategy.BacktestingStrategy):
	...
	def onStart(self):
		...
		self.__resultsFile.write("Instrument,Trade-Type,Entry-Date,Entry-Price,Quantity,Intraday-Exit-Date,Intraday-Exit-Price,Intraday-Exit-PorL,Exit-Date,Exit-Price,PorL,Current-Pos\n")
	...
	def onEnterOk(self, position):
		...
		self.__intraDayExitPrice[instrument] = 0.0
	...
	def onExitOk(self, position):
		...
		simulatedPOrL = "0.0"
		simulatedExitDate = "No-Exit"
		simulatedExitPrice = "0.0"
		if consts.SIMULATE_INTRA_DAY_EXIT and self.__intraDayExitPrice[instrument] != 0:
			simulatedPOrL = "%0.2f" % self.__intraDayExitPOrL[instrument]
			simulatedExitDate = self.__intraDayExitDate[instrument]
			simulatedExitPrice = "%0.2f" % self.__intraDayExitPrice[instrument]
		currPos = self.getBroker().getPositions()
		listOfCurrInstrs = list(currPos.keys())
		exitStr = simulatedExitDate + ',' + simulatedExitPrice + ',' + simulatedPOrL + ',' + exitDate + ',' + sellPrice + ',' + profitOrLoss + ',' + str(listOfCurrInstrs) + '\n'
			...
			if self.__longPos.get(instrument, None) and self.__longPos[instrument]:
				...
				if not self.__longPos[instrument].entryActive() and not self.__intraDayExits[instrument]:
					...
					if exitPrice != 0:
						...
						self.__intraDayExitPrice[instrument] = exitPrice
