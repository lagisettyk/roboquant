...
class BBSpread(strategy.BacktestingStrategy):
	...
	def onStart(self):
		...
		jsonStrategiesPath = os.path.join(os.path.dirname(__file__), 'json_strategies_trial')
	...
	def onBars(self, bars):
		...
		self.__sma_short = indicator.SMA(self.__closeDS, len(self.__closeDS), consts.SMA_SHORT_1)
		################################################################
	...
	def enterLongSignal(self, bar):
		...
		if "Cash_Flow" in self.__inpStrategy["BB_Spread_Call"] and "Cash_Flow_Check" in self.__inpStrategy["BB_Spread_Call"]["Cash_Flow"]:
			...
			#currentCFA = cashFlowArrayInLookback[-3:].sum()
			#previousDayCFA = cashFlowArrayInLookback[-4:-1].sum()
			currentCFA = cashFlowArrayInLookback[-1]
			previousDayCFA = cashFlowArrayInLookback[-2]
			#### Check corner case.....
			if (currentCFA > 0 and previousDayCFA > 0) or (currentCFA < 0 and previousDayCFA < 0):
				if (currentCFA > 0 and previousDayCFA > 0):
					cashFlowPercentChange = float((currentCFA - previousDayCFA)/(previousDayCFA)) * 100.0
				else:
					### Percent of negative numbers should be abs...
					cashFlowPercentChange = float((abs(currentCFA) - abs(previousDayCFA))/abs(previousDayCFA)) * 100.0
			else:
				cashFlowPercentChange = 9999 ### Since it is undefined..#if float(cashFlowArrayInLookback[(consts.CASH_FLOW_LOOKBACK_WINDOW - 1) * -1:].sum() / (consts.CASH_FLOW_LOOKBACK_WINDOW -1)) >= float(cashFlowArrayInLookback[consts.CASH_FLOW_LOOKBACK_WINDOW * -1:-1].sum() / (consts.CASH_FLOW_LOOKBACK_WINDOW -1)):
			...
			#if cashFlowArrayInLookback[-1] != cashFlowArrayInLookback.max():
			print "++++++++++++++++++======================== CF values: ", currentCFA, previousDayCFA, cashFlowPercentChange
			#if cashFlowArrayInLookback[-3:].sum() < cashFlowArrayInLookback[-4:-1].sum():
			if cashFlowPercentChange != 9999:
				if cashFlowPercentChange >= 20.0:
	...
	def enterShortSignal(self, bar):
		...
		if "Cash_Flow" in self.__inpStrategy["BB_Spread_Put"] and "Cash_Flow_Check" in self.__inpStrategy["BB_Spread_Put"]["Cash_Flow"]:
			##########################Momentum Oscillator Checks for proxying CASH FLOW/MONEY FLOW################
			#if  self.__mfi[-1] >= self.__mfi[-2]:
			#	self.__logger.debug("Trend & Money Flow Index check failed.")
			#	return False
			######################################### MFI check added by kiran ##################################
			if len(self.__priceDS) < consts.CASH_FLOW_LOOKBACK_WINDOW: 
				self.__logger.debug("Not enough entries for cashflow lookback")
			...
			if (currentCFA > 0 and previousDayCFA > 0) or (currentCFA < 0 and previousDayCFA < 0):
				...
				cashFlowPercentChange = 9999 ### Since it is undefined..#if float(cashFlowArrayInLookback[(consts.CASH_FLOW_LOOKBACK_WINDOW - 1) * -1:].sum() / (consts.CASH_FLOW_LOOKBACK_WINDOW -1)) >= float(cashFlowArrayInLookback[consts.CASH_FLOW_LOOKBACK_WINDOW * -1:-1].sum() / (consts.CASH_FLOW_LOOKBACK_WINDOW -1)):
			#if cashFlowArrayInLookback[-1] != cashFlowArrayInLookback.min():
				print "++++++++++++++++++======================== CF values: ", currentCFA, previousDayCFA, cashFlowPercentChange
			#if cashFlowArrayInLookback[-3:].sum() > cashFlowArrayInLookback[-4:-1].sum():
			if cashFlowPercentChange != 9999:
				if cashFlowPercentChange <= -20.0:
	...
	def exitLongSignal(self, bar):
		...
		if lowerBand > prevLowerBand:
			...
			# Since the first day's stop price is already adjusted...
			stopPrice = float(self.__entryDayStopPrice / self.__adjRatio)
			self.__adjStopPrice = stopPrice * self.__adjRatio
